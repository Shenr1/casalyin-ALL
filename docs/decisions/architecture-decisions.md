...
## ADR-006 仓库与物流功能（v2.0 规划中）

**日期：** 2026-04-14
**更新：** 2026-04-16（部分问题已确认）
**状态：** ⏳ 需求讨论中，不插队 v1.x 计划

### 核心定位

类 JD 自营仓储物流——供应商把货放到平台秘鲁仓库，平台负责发货。
...
| # | 问题 | 答案 |
|---|------|------|
| Q1 | 库存维度 | 挂在「产品」下，同一产品多经销商在不同区域，库存都在平台仓库 |
| Q2 | 操作角色 | 只有平台方（ADMIN 运营人员）能操作入库/出库，必须上传 PDF/图片凭证 |
| Q3 | 物流模式 | 第三方物流，基本状态（不走自有车队） |
| Q4（部分） | 展示角色 | 供应商=实时 Excel 库存+出入历史；买家=下单后基本物流状态 |

### 待确认（8 项，v2.0 立项前必须回答）

| # | 问题 |
|---|------|
| Q4 续 | 买家下单后 C 端如何追踪物流（"如果最初"后文被打断） |
| Q5 | SKU vs 产品：库存以 SKU 为单位还是产品为单位？多规格/多包装如何处理？ |
| Q6 | 供应商入库申请：手动提单还是上传 Excel？平台如何审核确认？ |
| Q7 | 库存预警：低于多少触发？通知推送给谁？ |
| Q8 | 库存为 0：前端展示缺货/预订/自动下架？ |
| Q9 | 物流状态档位：具体有几档？ |
| Q10 | 买家端展示形式：纯文字/地图轨迹/两者都有？ |
| Q11 | 仓库分区：是否需要库位管理（A 区/B 区）？ |
| Q12 | 退货/退款：是否支持？由谁处理？ |
